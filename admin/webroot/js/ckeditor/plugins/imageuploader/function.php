...
function loadImages() {
    ...
    if(file_exists($useruploadpath)){
        ...
        $dir = $useruploadpath;
        $files = glob("$dir*.{jpg,JPG,jpe,jpeg,JPEG,png,gif,ico}", GLOB_BRACE);
        ...
        for($i=count($files)-1; $i >= 0; $i--):
            ...
            $site=$site.$sub_url."albums/"; //local//
            ...
            if($file_style == "block") { ?>
                <div class="fileDiv"
                     onclick="showEditBar('<?php echo $image_url; ?>','<?php echo $image_height; ?>','<?php echo $count; ?>','<?php echo $image_basename; ?>','<?php echo $unique_id; ?>');"
                     ondblclick="showImage('<?php echo $image_url; ?>','<?php echo $image_height; ?>','<?php echo $image_basename; ?>','<?php echo $unique_id; ?>');"
            ...
            <?php } elseif($file_style == "list") { ?>
                <div class="fullWidthFileDiv"
                    ...
                    <div class="qEditIconsDiv">
                        <img title="Delete File" src="img/cd-icon-qtrash.png" class="qEditIconsImg" onclick="window.location.href = 'imgdelete.php?img=<?php echo $image_basename; ?>&unique_id=<?php echo $unique_id; ?>'">
                    </div>
