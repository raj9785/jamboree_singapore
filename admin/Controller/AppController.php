...
class AppController extends Controller {
    ...
    function gen_slug($str, $table = '', $id = '', $slug_field = '') {
# special accents
        ...
        $slug_field = $slug_field ? $slug_field : "slug";
        if ($id) {
            $cond = array(
                $table . '.' . $slug_field . ' LIKE' => "" . $slug . "%",
                $table . '.id !=' => $id,
            );
        } else {
            $cond = array(
...
//parent::beforeFilter();
        ...
        $this->set('schedules_types', $this->schedules_types);
    }
    ...
    public function global_logs($table_name, $table_id, $action_type, $text_action, $json_data) {
        ...
        $arr['Logs']['action_taken_by_name'] = $this->Auth->user('first_name') ? $this->Auth->user('first_name') . " " . $this->Auth->user('last_name') : $this->Auth->user('last_name');
        $this->Logs->save($arr);
        return true;
    }
